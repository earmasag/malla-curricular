...
const MallaContent = () => {
  ...
  const [hoveredMateria, setHoveredMateria] = React.useState<string | null>(
    null
  );
  ...
  const handleResetProgreso = () => {
    if (
      window.confirm(
        "¿Estás seguro que deseas borrar todo tu progreso? Esta acción no se puede deshacer."
      )
    ) {
      resetProgreso();
    }
  ...
  return (
    <div className="flex relative h-screen w-screen bg-gray-100 font-sans m-0 overflow-hidden">
      ...
      <header
        // 2. We toggle the state when the user clicks the header
      ...
      >
        {/* 3. Conditional rendering: Only show the title if it is expanded */}
        ...
        <div className="flex items-center gap-2 text-sm font-medium text-gray-700">
          ...
          {/* Botón Borrar: Only show the whole button if expanded */}
          {isExpanded && (
            <button
              onClick={(e) => {
                e.stopPropagation(); // 4. Prevents the header click event from triggering when clicking this button
                handleResetProgreso();
              }}
